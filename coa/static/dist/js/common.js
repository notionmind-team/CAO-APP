...
$('#chk_all_oder_items').click(function () 
{
  if ($('#chk_all_oder_items').is(':checked') == true) 
  {
    $(".chk_order").prop('checked', true);
    $("#order_itm_btn").show()
    $('.chk_order').each( function() 
    {
      $("#quantity_"+$(this).val()).attr("readonly", false);
      $("#quantity_"+$(this).val()).attr("required", true); 
    });
  }
  else
  {
    ...
    $('.chk_order').each( function() 
    {
      $("#quantity_"+$(this).val()).attr("readonly", true);
      $("#quantity_"+$(this).val()).attr("required", false);  
    });
  }
});
...
{
  if($('.chk:checked').length == $('.chk_order').length)
  {
    $('#chk_all_oder_items').prop('checked', true);
    $("#quantity_"+$(this).val()).attr("readonly", false);
    $("#quantity_"+$(this).val()).attr("required", true); 

  }
  else if($('.chk_order:checked').length == 0)
  {
    $("#order_itm_btn").hide()
    $("#quantity_"+$(this).val()).attr("readonly", true);
    $("#quantity_"+$(this).val()).attr("required", false); 
  ...
  {
    $('#chk_all_oder_items').prop('checked', false);
    $("#order_itm_btn").show()
...
$('#order_itm_btn').click(function() 
{
  var validation_order = true;
  $('.chk_order').each( function() 
  {
      if($(this).prop("checked") == true)
      {
        if($("#quantity_"+$(this).val()).val() == '')
        {
          $("#quantity_error_"+$(this).val()).text('Please enter value.');
          $("#quantity_"+$(this).val()).addClass('is-invalid')
          validation_order = false
        }
        else if($("#quantity_"+$(this).val()).val() < 0)
        {
          $("#quantity_error_"+$(this).val()).text('Please enter valid quantity');
          $("#quantity_"+$(this).val()).addClass('is-invalid')
          validation_order = false
        }
        else
        {
          $("#quantity_error_"+$(this).val()).text('');
          $("#quantity_"+$(this).val()).removeClass('is-invalid')
        }
      }
      else
      {
