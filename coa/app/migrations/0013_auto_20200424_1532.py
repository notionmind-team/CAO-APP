# Generated by Django 3.0.5 on 2020-04-24 15:32

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    dependencies = [
        ('app', '0012_item_image'),
    ]

    operations = [
        migrations.CreateModel(
            name='WeeklyItemsAvg',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('avg', models.FloatField()),
                ('ratio', models.FloatField()),
                ('createdAt', models.DateTimeField(auto_now_add=True, null=True)),
                ('item', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='app.Item')),
            ],
            options={
                'verbose_name': 'Item Weekly Avg',
                'verbose_name_plural': 'Item Weekly Avg',
            },
        ),
        migrations.CreateModel(
            name='WeeklySales',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('mon', models.FloatField()),
                ('tue', models.FloatField()),
                ('wed', models.FloatField()),
                ('thur', models.FloatField()),
                ('fri', models.FloatField()),
                ('sat', models.FloatField()),
                ('sun', models.FloatField()),
                ('tot', models.FloatField()),
                ('createdAt', models.DateTimeField(auto_now_add=True, null=True)),
                ('category', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='app.Category')),
                ('store', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='app.Store')),
                ('subcategory', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='app.Subcategory')),
            ],
            options={
                'verbose_name': 'Weekly Sales Data',
                'verbose_name_plural': 'Weekly Sales Data',
            },
        ),
        migrations.CreateModel(
            name='WeeklyItemsData',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('mon', models.FloatField()),
                ('tue', models.FloatField()),
                ('wed', models.FloatField()),
                ('thur', models.FloatField()),
                ('fri', models.FloatField()),
                ('sat', models.FloatField()),
                ('sun', models.FloatField()),
                ('createdAt', models.DateTimeField(auto_now_add=True, null=True)),
                ('itemavg', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='app.WeeklyItemsAvg')),
            ],
        ),
        migrations.AddField(
            model_name='weeklyitemsavg',
            name='week',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='app.WeeklySales'),
        ),
    ]
